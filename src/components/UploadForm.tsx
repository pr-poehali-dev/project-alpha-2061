...
export default function UploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !oldKp || !referenceKp) {
      toast({
        title: "Заполните все поля",
        description: "Укажите имя, email и загрузите оба файла",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Мы получили ваши файлы и скоро пришлём результат",
      })
    } catch (err) {
      toast({
  ...
  if (isDone) {
    return (
      <div className="w-full max-w-xl rounded-3xl border border-white/20 bg-white/10 p-8 backdrop-blur-md text-center flex flex-col items-center gap-4">
        ...
        <p className="text-white/70">
          Мы обработаем ваше КП и пришлём готовый вариант на {email}
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        <Label htmlFor="phone" className="text-white/90">Телефон (необязательно)</Label>
        <Input
          id="phone"
          value={phone}
