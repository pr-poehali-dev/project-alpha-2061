    ...
    content = [
        {
            ...
            'text': (
                ...
                'КРИТИЧЕСКИ ВАЖНЫЕ ТЕХНИЧЕСКИЕ ОГРАНИЧЕНИЯ (документ рендерится через xhtml2pdf, '
                'который поддерживает только ограниченное подмножество CSS 2.1):\n'
                '- ЗАПРЕЩЕНО использовать display:flex, display:grid, position:sticky, position:fixed, CSS Grid, CSS переменные — они НЕ РАБОТАЮТ и ломают вёрстку.\n'
                '- Для расположения блоков рядом используй ТОЛЬКО <table><tr><td> вёрстку или display:inline-block с float:left/float:right.\n'
                '- Для позиционирования элементов друг над другом используй position:relative + position:absolute с точными px/mm координатами.\n'
                '- Все размеры задавай в px, mm или pt (не используй vw/vh/%, кроме width в table).\n'
                '- Каждая секция/блок — это <div> или <table> с явным width, padding, background-color, border-radius, border.\n'
                '- Используй только inline CSS и <style> в <head> (без внешних файлов и шрифтов из интернета).\n'
                '- Документ формата A4 (используй @page { size: A4; margin: 0; } и корневой контейнер width:210mm).\n'
                '- Не используй box-shadow, gradient — они не рендерятся. Используй только сплошные background-color.\n\n'
